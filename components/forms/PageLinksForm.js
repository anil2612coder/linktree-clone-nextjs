...
import MediaUpload from "@/libs/upload";
...
export default function PageLinksForm({page,user}) {
  ...
  function handleUpload(uploadImageLink, linkKeyForUpload) {
   
      setLinks(prevLinks => {
        const newLinks = [...prevLinks];
        newLinks.forEach((link,index) => {
          if (link.key === linkKeyForUpload) {
            link.icon = uploadImageLink;
              ...
              <div key={l.key} className="mt-8 md:flex gap-6 items-center">
                ...
                <div className="text-center">
                  ...
                  <div>
                       ...
                       <MediaUpload
                       onChange={(value)=> handleUpload(value, l.key)}>
                    <label  className="border mt-2 p-2 flex items-center gap-1 text-gray-600 cursor-pointer mb-2 justify-center">
                    <FaCloudArrowUp />
